...
export class PointerTool extends BaseTool {
    ...
    if (this.equalizeMode && !hasSkeletonPointUnderCursor) {
      ...
      if (handleInfo && positionedGlyph) {
        ...
        await runDragRoutingOrchestration({
          ...
          eventStream,
          initialEvent,
          objectKind: "regularEqualizeHandle",
          handleInfo,
          positionedGlyph,
          editableHandleInfo,
        });
    ...
    await sceneController.editGlyph(async (sendIncrementalChange, glyph) => {
      ...
      for await (const event of eventStream) {
        ...
        if (skeletonEditState) {
          ...
          const prefixedSkeletonChanges = skeletonChanges.prefixed([
            "layers",
            editLayerName,
            "glyph",
          ]);
          ...
          const prefixedCustomDataChange = customDataChange.prefixed([
            "layers",
            editLayerName,
          ]);
      ...
      for (const data of Object.values(layersData)) {
        ...
        for (const eh of editableHandles) {
          ...
          const equalizeInfo = layerPath
            ? getEqualizeHandleInfoForPointIndex(layerPath, eh.pointIndex)
            : null;
  ...
  async _handleArrowKeysForEqualizePathHandles(delta, pointSelection) {
    ...
    for (const pointIndex of pointSelection) {
      const info = getEqualizeHandleInfoForPointIndex(basePath, pointIndex);
      if (info) {
