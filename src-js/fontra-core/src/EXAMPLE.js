...
// Calculate tension percentage for off-curve points
export function calculateTension(offCurvePoint, onCurvePoint, nextOnCurvePoint) {
  // Calculate the actual handle length (direct distance from on-curve to off-curve point)
  const handleDx = offCurvePoint.x - onCurvePoint.x;
  const handleDy = offCurvePoint.y - onCurvePoint.y;
  const handleLength = Math.hypot(handleDx, handleDy);
  
  // Calculate the chord length (distance between on-curve points)
  const chordDx = nextOnCurvePoint.x - onCurvePoint.x;
  const chordDy = nextOnCurvePoint.y - onCurvePoint.y;
  const chordLength = Math.hypot(chordDx, chordDy);
  
  if (chordLength === 0) return 0;
  
  // Calculate the ideal handle length for a smooth curve (approximately 1/3 of chord)
  const idealHandleLength = chordLength / 3;
  
  // Calculate tension as percentage of ideal length
  // 0% when handle length is 0, 100% when handle length equals ideal length
  const tension = (handleLength / idealHandleLength) * 100;
  
  // Clamp to reasonable range (0-150% allows for some overshoot)
  return Math.max(0, Math.min(150, tension));
}
...
export function formatDistanceTensionAngle(distance, tension, angle) {
  return `${distance.toFixed(1)} / ${tension.toFixed(0)}% / ${angle.toFixed(1)}°`;
}
...
export function drawOffCurveDistanceVisualization(context, positionedGlyph, parameters, model, controller) {
  ...
  // Find the index of the on-curve point (distanceInfo.point) in the path
  // This is needed because when an off-curve point is selected, we need to
  // determine the next point based on the associated on-curve point, not the selected point
  let onCurvePointIndex = null;
  for (let i = 0; i < path.coordinates.length; i += 2) {
    const point = { x: path.coordinates[i], y: path.coordinates[i + 1] };
    if (point.x === distanceInfo.point.x && point.y === distanceInfo.point.y) {
      onCurvePointIndex = i / 2;
      break;
    }
  }
  
  // Get the next on-curve point for tension calculation
  // Use the on-curve point index instead of the selected point index
  let nextPoint = null;
  if (onCurvePointIndex !== null) {
  ...
  for (const distanceData of distanceInfo.distances) {
    ...
    // Calculate tension and angle
    let tension = 0;
    let angle = 0;
    if (nextPoint) {
      tension = calculateTension(offCurvePoint, distanceInfo.point, nextPoint);
      // Clamp tension to 0-150 range
      tension = Math.max(0, Math.min(150, tension));
      angle = calculateOffCurveAngle(offCurvePoint, distanceInfo.point);
    }
