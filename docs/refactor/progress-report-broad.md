...
## Phase 6 - Cross-Cut: Final pointer dead-code cleanup before optimization

- Problem: After Phase 5/6 routing was complete, `edit-tools-pointer.js` still carried dead in-scope drag/nudge method stubs and dead rib-helper wrappers. They were no longer part of live routing, but they obscured the transport-only boundary and added noise ahead of optimization work.
- Code analysis:
  - Updated `src-js/views-editor/src/edit-tools-pointer.js`.
  - Removed dead in-scope pointer stubs that only returned `false`:
    - `_handleArrowKeysLegacy(...)`
    - `_handleDragMixedSelection(...)`
    - `_handleDragSkeletonPoints(...)`
    - `_handleDragRibPoint(...)`
    ...
    - `_handleArrowKeysForEqualizeSkeletonHandles(...)`
    - `_handleArrowKeysForEqualizePathHandles(...)`
  - Removed dead rib-wrapper helpers that were no longer referenced after adapter migration:
    - `_findHandlesForRibPointFromSkeleton(...)`
    - `_findAdjacentHandlesForRibPoint(...)`
    - `_getSelectedRibPointInfo(...)`
  - Removed now-unused imports for rib interpolation helper wrappers from `edit-tools-pointer.js`.
  - Verification:
    - `node --check src-js/views-editor/src/edit-tools-pointer.js`
    - `npm run -s bundle`
- Comparison: Yes. This cleanup does not change live drag/nudge behavior; it removes only unreferenced pointer-side leftovers so the file more accurately reflects the completed unified-behavior architecture and is easier to optimize next.
- Manual test results: NOT RUN in this terminal session (cleanup is structural; no UI pass performed here).
- Undo/redo verification: NOT RUN in this terminal session (no behavior-path changes were made in this cleanup).
