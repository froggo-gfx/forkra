...
- **Copy / cut / paste of skeleton is EXCLUDED (deferred).** `StaticGlyph.fromObject` drops `customData` (verified), so a correct implementation needs the donor's explicit `skeletonDataByLayer` clipboard field plus paste-side frozen-contour stripping + `allocateSkeletonIds`/`editSkeleton` regeneration; doing it safely needs the editor test harness / side-by-side donor and risks glyph corruption otherwise. **Delete** of skeleton points IS implemented. Whole-glyph paste-replace carries skeleton via `Layer.customData`; layer/selection paste is lossy-but-safe. Full detail and follow-up in `docs/superpowers/notes/ws16-parity-audit.md`. The tested `allocateSkeletonIds` helper (Task 1) is in place for the follow-up.
- **Decompose and selection-info** required no code change: decompose appends at the path end and never touches `customData` (skeleton survives); selection-info ignores `skeletonPoint` keys (no crash). Both are recorded as verify-only passes in the audit note.
